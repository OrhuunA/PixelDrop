...
- Daily goal/streak and the 7-day history are computed from the last 14
  days kept in `config.json`'s `history` field (date → cups drunk); the
  day-rollover check now runs not just at startup but on every one-second
  `_tick`, so it resets correctly at midnight even if the app has been
  open all night.
- Languages live in the `STRINGS` dictionary (inside main.py, "tr"/"en");
  the choice is stored in `config.json`'s `language` field. Widget/tray
  text re-reads the current language on every refresh; the settings
  window's static labels are updated by reopening the window when the
  language changes.
